...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 relative overflow-hidden font-sans">
      ...
      <div className="bg-emerald-500/5 w-[40rem] h-[40rem] rounded-full blur-[160px] absolute -bottom-48 -right-48 pointer-events-none z-0" />

      <div className="max-w-6xl mx-auto space-y-6 relative z-10">

        {/* Header Dashboard */}
        <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-900/80 backdrop-blur border border-slate-800 p-6 rounded-2xl shadow-xl gap-4">
          <div className="flex items-center space-x-4">
            <div className="p-3 bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 rounded-xl">
              <Shield className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-black bg-gradient-to-r from-slate-100 to-indigo-200 bg-clip-text text-transparent">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto px-4 py-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold shadow-md cursor-pointer"
            >
              <Scale className="w-4 h-4" /> Kelola Portal Perada
            </button>
            <button
              onClick={() => window.location.href = '/admin/trantib'}
              className="w-full sm:w-auto px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold shadow-md cursor-pointer"
            >
              <Shield className="w-4 h-4" /> Kelola Portal Trantib
            </button>
            <button
              onClick={() => window.location.href = '/admin/linmas'}
              className="w-full sm:w-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold shadow-md cursor-pointer"
            >
              <Shield className="w-4 h-4" /> Kelola Portal Linmas
            </button>
            <button
              onClick={fetchReports}
              className="w-full sm:w-auto px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700/80 rounded-xl transition-all flex items-center justify-center gap-2 text-xs font-bold shadow-inner cursor-pointer"
            >
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="text-center p-16 bg-slate-900/30 border border-slate-900 rounded-2xl flex flex-col items-center justify-center gap-3">
              <RefreshCw className="w-8 h-8 text-indigo-500 animate-spin" />
              <div className="text-sm font-semibold text-slate-400">Menghubungkan ke core database dev.db...</div>
            </div>
          ) : reports.length === 0 ? (
            <div className="text-center p-16 bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-850 text-slate-400 text-sm space-y-2">
              <Clipboard className="w-12 h-12 text-slate-600 mx-auto" />
              <p className="font-bold text-slate-300">Belum ada data pengaduan masuk</p>
            ...
            reports.map((report) => (
              <div 
                key={report.id_tiket} 
                className="bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 hover:border-slate-800 rounded-2xl p-5 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-6 transition-all duration-300 shadow-lg relative"
              >
                
                {/* Info Tiket & Pelapor (Kiri) */}
                <div className="border-b md:border-b-0 md:border-r border-slate-800 pb-4 md:pb-0 md:pr-6 space-y-3 flex flex-col justify-between">
                  <div className="space-y-1.5">
                    ...
                    <h3 className="text-base font-bold text-slate-200">{report.kategori_masalah}</h3>
                    <p className="text-xs text-slate-400">
                      Pelapor: <span className="font-bold text-slate-300">{report.nama_pelapor}</span>
                    </p>
                    <p className="text-xs text-slate-400">
                      Kontak: <span className="font-bold text-slate-300">{report.nomor_whatsapp}</span>
                    </p>
                  </div>
                  
                  <div className="text-[10px] text-slate-500 flex items-center gap-1 font-medium pt-2 border-t border-slate-850">
                ...
                <div className="flex flex-col justify-between space-y-4 md:px-2">
                  ...
                  {report.foto_bukti && (
                    <div className="space-y-1.5">
                      ...
                      <div className="w-24 h-16 rounded-lg overflow-hidden border border-slate-800 bg-slate-950 relative group cursor-zoom-in" onClick={() => {
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-950 hover:bg-slate-900 text-xs font-bold text-emerald-400 rounded-xl border border-slate-850 cursor-pointer transition-colors"
                    >
                ...
                <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-800 flex flex-col justify-between gap-4">
                  <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        {report.disposisi && (
                          <p className="text-[9px] text-slate-500">
                            Oleh: {report.disposisi.nama_admin} | Kedaruratan: <span className={`font-bold ${report.disposisi.kedaruratan === 'Darurat' ? 'text-rose-400' : report.disposisi.kedaruratan === 'Sedang' ? 'text-amber-400' : 'text-emerald-400'}`}>{report.disposisi.kedaruratan}</span>
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <div className="flex gap-2 pt-2 border-t border-slate-850/60">
                    <button
                      ...
                      className="px-3 py-1.5 bg-rose-600/10 hover:bg-rose-600/25 border border-rose-600/20 text-rose-400 text-xs rounded-xl font-bold flex items-center justify-center gap-1 cursor-pointer transition-colors shadow"
                    ...
                    {report.status_laporan === "Pending" ? (
                      <button
                        ...
                        className="flex-1 py-1.5 bg-gradient-to-r from-indigo-600 to-emerald-600 hover:from-indigo-500 hover:to-emerald-500 text-white text-xs rounded-xl font-bold flex items-center justify-center gap-1.5 transition-all shadow-md active:scale-95 cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-indigo-300 text-xs rounded-xl font-bold flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
                      >
      ...
      {isModalOpen && selectedReport && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          ...
          {/* Modal Container */}
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full shadow-2xl relative z-10 transition-all duration-300 overflow-hidden font-sans">
            ...
            <div className="bg-slate-950 px-6 py-4 border-b border-slate-800 flex justify-between items-center">
              ...
              <button 
                ...
                className="text-slate-400 hover:text-white bg-slate-900 border border-slate-800 p-1 rounded-lg transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmitDisposisi}>
              ...
              <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-indigo-400 font-mono font-bold outline-none"
                    />
                  </div>
                  ...
                  {modalMode === 'create' ? (
                    <>
                      <select
                        ...
                        className={`w-full bg-slate-950 border ${formErrors.namaAdmin ? 'border-rose-500' : 'border-slate-800'} rounded-xl px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500`}
                      >
                ...
                <div className="space-y-1.5">
                  ...
                  {modalMode === 'create' ? (
                    ...
                    <div className="px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs font-bold text-indigo-400">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button
                          ...
                          className={`py-2 px-3 text-xs font-bold rounded-xl border text-center transition-all cursor-pointer ${
                            disposisiForm.kedaruratan === level.id ? level.active : `bg-slate-950 border-slate-800 ${level.color}`
                          }`}
                        >
                          {level.label}
                        </button>
                  ...
                  {modalMode === 'create' ? (
                    <>
                      <textarea
                        ...
                        className={`w-full bg-slate-950 border ${formErrors.catatan ? 'border-rose-500' : 'border-slate-800'} rounded-xl px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500 resize-y`}
                      />
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {QUICK_TEMPLATES.map((tmpl, idx) => (
                            <button
                              ...
                              className="text-[9px] bg-slate-950 hover:bg-slate-850 text-slate-400 hover:text-slate-200 px-2 py-1 rounded border border-slate-850 hover:border-slate-700 text-left transition-colors truncate max-w-full cursor-pointer"
                              title={tmpl}
                            >
                              Template {idx + 1}
                            </button>
                    ...
                    <div className="bg-slate-950 border border-slate-850 p-4 rounded-xl text-xs text-slate-300 italic leading-relaxed border-l-4 border-l-indigo-500">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-500 outline-none"
                  />
                </div>
              ...
              <div className="bg-slate-950 px-6 py-4 border-t border-slate-800 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer"
                >
                ...
                {modalMode === 'create' && (
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-emerald-600 hover:from-indigo-500 hover:to-emerald-500 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow disabled:opacity-50 cursor-pointer"
                  >
                    {submitting ? (
                      <>
